...
# Simple updater script for mydyndnsvr running on your own server. Change the
...
TOKEN="$1"
if [ -z "$TOKEN" ]; then
	echo -e "Usage: mydyndnsrv-update.sh TOKEN\n"
	exit 1
fi
...
OLD_IP4=""
STATUS_IP6=""
if [ -e /tmp/mydyndnsrv-update4 ]; then
	OLD_IP4=`cat /tmp/mydyndnsrv-update4`
fi
if [ -n "$IP4" ]; then
	if [ "$IP4" != "$OLD_IP4" ]; then
		...
		if [ "$STATUS_IP4" == "accepted" ]; then
			...
			echo $IP4 > /tmp/mydyndnsrv-update4
		else
...
OLD_IP6=""
STATUS_IP6=""
if [ -e /tmp/mydyndnsrv-update6 ]; then
	OLD_IP6=`cat /tmp/mydyndnsrv-update6`
fi
if [ -n "$IP6" ]; then
	if [ "$IP6" != "$OLD_IP6" ]; then
		...
		if [ "$STATUS_IP6" == "accepted" ]; then
			...
			echo $IP6 > /tmp/mydyndnsrv-update6
		else
